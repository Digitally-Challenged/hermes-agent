...
# Floor for the schema bytes the compact profile must save across the telegram
# toolset. Measured at ~10.2 KB with the five compacted tools; asserted 10 KB
# so the test fails on a real regression, not on a tool set that legitimately
# differs by one check_fn-gated tool between environments.
MIN_TELEGRAM_BYTES_SAVED = 10_000
...
class TestTelegramToolsetProfiles:
    ...
    def test_total_schema_bytes_drop_by_at_least_10kb(self, profiles):
        off, on = profiles
        saved = _schema_bytes(off) - _schema_bytes(on)
        assert saved >= MIN_TELEGRAM_BYTES_SAVED, (
            f"compact profile saved only {saved} bytes across "
            f"{len(off)} telegram tools; expected >= {MIN_TELEGRAM_BYTES_SAVED}"
        )
