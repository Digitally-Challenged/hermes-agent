...
| `HERMES_CRON_TIMEOUT` | **Deprecated** — use `cron.timeout_seconds` in `config.yaml` instead. Inactivity timeout for cron job agent runs in seconds (default: `600`). The agent can run indefinitely while actively calling tools or receiving stream tokens — this only triggers when idle. Set to `0` for unlimited. Only consulted when `cron.timeout_seconds` is unset. |
